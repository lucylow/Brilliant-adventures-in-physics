...
export function Card({ children, onPress, accessibilityLabel, style, reducedMotion = false }: PropsWithChildren<{ onPress?: () => void; accessibilityLabel?: string; style?: StyleProp<ViewStyle>; reducedMotion?: boolean }>) {
  ...
  const content = <View style={[{ padding: spacing.md, borderRadius: radius.lg, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border }, style]}>{children}</View>;
...
export function PrimaryButton({ label, onPress, disabled = false, reducedMotion = false }: { label: string; onPress: () => void; disabled?: boolean; reducedMotion?: boolean }) {
  const colors = useColors();
  return <MotionPressable accessibilityRole="button" accessibilityLabel={label} accessibilityState={{ disabled }} disabled={disabled} onPress={onPress} reducedMotion={reducedMotion} style={({ pressed }) => ({ minHeight: 50, padding: 14, borderRadius: radius.md, alignItems: "center", justifyContent: "center", backgroundColor: pressed ? colors.primaryPressed : colors.primary, opacity: disabled ? 0.45 : 1 })}><Text style={{ color: colors.onPrimary, fontWeight: "800", textAlign: "center" }}>{label}</Text></MotionPressable>;
}
