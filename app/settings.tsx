...
import { clearRetryQueue, formatLastSave, formatRetryItemAge, formatRetryResult, getLastSave, getRetryCount, getRetryItems, markLastSave, retryQueue, RETRY_QUEUE_DISCARD_COPY, RETRY_QUEUE_DISCARDED_COPY, type RetryItem } from "@/lib/retry-queue";
  ...
  return (
    <ScreenContainer edges={["top", "bottom", "left", "right"]} className="p-5">
      <ScrollView contentContainerStyle={{ paddingBottom: 32 }}>
        ...
        <Card style={{ marginTop: 12 }}><Text style={{ color: colors.foreground, fontSize: 18, fontWeight: "800" }}>Offline saves</Text><Text style={{ color: colors.muted, marginTop: 5, lineHeight: 20 }}>{retryCount > 0 ? `${retryCount} autosave${retryCount === 1 ? "" : "s"} waiting to retry.` : "No offline autosaves are waiting."}</Text><Text style={{ color: colors.muted, marginTop: 4 }}>{formatLastSave(lastSave)}</Text>{retryItems.length > 0 && <View accessible accessibilityLabel={`${retryItems.length} pending autosave${retryItems.length === 1 ? "" : "s"}`} style={{ marginTop: 8 }}>{retryItems.slice(0, 3).map((item, index) => <Text key={item.id} style={{ color: colors.muted, lineHeight: 20 }}>Draft {index + 1} · queued {formatRetryItemAge(item.queuedAt, Date.now())}</Text>)}{retryItems.length > 3 && <Text style={{ color: colors.muted }}>Plus {retryItems.length - 3} more queued.</Text>}</View>}<View style={{ marginTop: 12 }}><SecondaryButton label={retrying ? "Retrying offline saves…" : "Retry offline saves"} onPress={retryPendingSaves} /></View>{retryCount > 0 && <View style={{ marginTop: 8 }}><SecondaryButton label="Discard queued offline saves" onPress={confirmDiscardPendingSaves} /></View>}{retryMessage && <Text accessibilityLiveRegion="assertive" style={{ marginTop: 10, color: colors.warning }}>{retryMessage}</Text>}</Card>
