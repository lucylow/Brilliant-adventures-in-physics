...
import { clearRetryQueue, formatLastSave, formatRetryResult, getLastSave, getRetryCount, markLastSave, retryQueue, RETRY_QUEUE_DISCARD_COPY, RETRY_QUEUE_DISCARDED_COPY } from "@/lib/retry-queue";
...
export default function SettingsScreen() {
  ...
  const confirmDiscardPendingSaves = () => { if (retrying || retryCount === 0) return; Alert.alert("Discard offline saves?", RETRY_QUEUE_DISCARD_COPY, [{ text: "Keep saves", style: "cancel" }, { text: "Discard", style: "destructive", onPress: () => { setRetrying(true); void clearRetryQueue().then(() => { setRetryCount(0); setLastSave(null); setRetryMessage(RETRY_QUEUE_DISCARDED_COPY); }).catch(() => setRetryMessage("We could not discard queued offline saves. Your work remains available.")).finally(() => setRetrying(false)); } }]); };
  return (
