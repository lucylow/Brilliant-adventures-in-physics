...
export default function SettingsScreen() {
  ...
  useEffect(() => { let active = true; void loadPreferencesWithStatus().then((result) => { if (!active) return; setPreferences(result.preferences); if (result.recovered) setLoadMessage(tr("settings.preferencesRecovered")); }).catch(() => { if (active) setLoadMessage(tr("settings.preferencesUnavailable")); }); void Promise.all([loadRetryQueueWithStatus(), getLastSave()]).then(([queueResult, savedAt]) => { if (!active) return; setRetryCount(queueResult.items.length); setRetryItems(queueResult.items); setLastSave(savedAt); if (queueResult.recovered) setRetryMessage(queueResult.reason === "malformed" ? tr("settings.retryQueueRecovered") : tr("settings.retryDetailsUnavailable")); }).catch(() => { if (active) setRetryMessage(tr("settings.retryDetailsUnavailable")); }); return () => { active = false; }; }, [tr]);
  useEffect(() => { let active = true; const unsubscribe = subscribeAutosaveSync((event) => { if (!active) return; setLastSyncCount(event.saved); setLastSave(new Date(event.occurredAt).toISOString()); void loadRetryQueueWithStatus().then((result) => { if (!active) return; setRetryCount(result.items.length); setRetryItems(result.items); if (result.recovered) setRetryMessage(result.reason === "malformed" ? tr("settings.retryQueueRecovered") : tr("settings.retryDetailsUnavailable")); }).catch(() => { if (active) setRetryMessage(tr("settings.retryDetailsUnavailable")); }); }); return () => { active = false; unsubscribe(); }; }, [tr]);
  ...
  const retryPendingSaves = () => { if (retrying) return; setRetrying(true); setRetryProgress(null); setRetryMessage(null); void retryQueue(async (item) => { await saveDraft({ id: item.id, data: item.payload, updatedAt: item.queuedAt }); }, (progress) => setRetryProgress(progress)).then(async (result) => { if (result.saved > 0) { await markLastSave(); publishAutosaveSync(result.saved); } setRetryCount(result.remaining); setRetryItems(result.remaining ? await getRetryItems() : []); setLastSave(result.saved > 0 ? new Date().toISOString() : lastSave); setRetryProgress(null); setRetryMessage(formatRetryResult(result.saved, result.remaining)); }).catch(() => { setRetryProgress(null); setRetryMessage(tr("settings.retryFailed")); }).finally(() => setRetrying(false)); };
  ...
  const retryOnePendingSave = (item: RetryItem, index: number) => { if (retrying) return; setRetrying(true); setRetryMessage(null); void retryOneItem(item.id, async (queued) => { await saveDraft({ id: queued.id, data: queued.payload, updatedAt: queued.queuedAt }); }).then(async (result) => { setRetryCount(result.remaining); setRetryItems(await getRetryItems()); if (result.saved) { await markLastSave(); setLastSave(new Date().toISOString()); publishAutosaveSync(1); } setRetryMessage(formatRetryItemResult(index, result.saved, result.remaining)); }).catch(() => setRetryMessage(tr("settings.retryItemFailed", { index: index + 1 }))).finally(() => setRetrying(false)); };
  return (
