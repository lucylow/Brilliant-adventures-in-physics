...
import { manualNetworkCheckMessage, networkStateToStatus, networkStatusLabel, type NetworkStatus } from "@/lib/network";
...
import { clearRetryQueue, formatLastSave, formatRetryItemAge, formatRetryItemResult, formatRetryResult, getLastSave, getRetryCount, getRetryItems, markLastSave, removeRetryItem, retryOneItem, retryQueue, RETRY_ITEM_DISCARD_COPY, RETRY_ITEM_DISCARDED_COPY, RETRY_QUEUE_DISCARD_COPY, RETRY_QUEUE_DISCARDED_COPY, type RetryItem } from "@/lib/retry-queue";
...
export default function SettingsScreen() {
  ...
  const checkConnection = () => { if (checkingNetwork) return; setCheckingNetwork(true); void Network.getNetworkStateAsync().then((state) => { const status = networkStateToStatus(state); setNetworkStatus(status); setRetryMessage(manualNetworkCheckMessage(status)); }).catch(() => { setNetworkStatus("unknown"); setRetryMessage(manualNetworkCheckMessage("unknown")); }).finally(() => setCheckingNetwork(false)); };
