...
export default function OnboardingSummaryScreen() {
  ...
  useEffect(() => { let active = true; void loadOnboardingWithStatus().then((result) => { if (!active) return; setProfile(result.profile); setLoadRecovered(result.recovered); }).catch(() => { if (active) setLoadFailed(true); }); return () => { active = false; }; }, []);
