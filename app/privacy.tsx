...
import { buildLocalDataShareText, clearAllLocalData, formatLocalDataSummary, formatPrivacyActivityTimestamp, getLocalDataSummary, loadPrivacyActivityWithStatus, localSummaryFileUri, recordPrivacyActivity, type LocalDataSummary, type PrivacyActivityEvent } from "@/lib/privacy";
...
export default function PrivacyScreen() {
  ...
  const refresh = () => { void Promise.all([getLocalDataSummary(), loadPrivacyActivityWithStatus()]).then(([next, result]) => { setSummary(next); setActivity(result.events); setActivityFallback(result.usedFallback); setShareMessage(null); }).catch(() => setShareMessage(loadFailureMessage)); };
  useEffect(() => { let active = true; void Promise.all([getLocalDataSummary(), loadPrivacyActivityWithStatus()]).then(([next, result]) => { if (!active) return; setSummary(next); setActivity(result.events); setActivityFallback(result.usedFallback); setShareMessage(null); }).catch(() => { if (active) setShareMessage(loadFailureMessage); }); return () => { active = false; }; }, [loadFailureMessage]);
  ...
  const clear = () => Alert.alert(tr("privacy.clearTitle"), tr("privacy.clearBody"), [{ text: tr("privacy.cancel"), style: "cancel" }, { text: tr("privacy.clear"), style: "destructive", onPress: () => { void clearAllLocalData().then(async () => { await recordPrivacyActivity("clear", "success"); await refresh(); }).catch(async () => { await recordPrivacyActivity("clear", "failure"); await loadPrivacyActivityWithStatus().then((result) => { setActivity(result.events); setActivityFallback(result.usedFallback); }).catch(() => undefined); setShareMessage(tr("privacy.clearFailed")); }); } }]);
  const share = async () => {
    if (sharing) return;
    setSharing(true);
