...
export default function LabScreen() {
  ...
  return <ScreenContainer className="p-5"><ScrollView contentContainerStyle={{ paddingBottom: 32 }}><SectionHeader title="Physics Lab" subtitle="Manipulate a model and inspect verified values." /><Card><ProjectileVisual points={points} peakHeight={result.peakHeight} running={running} /><View style={{ marginTop: 18 }}><Slider label="Launch speed (m/s)" value={speed} min={4} max={32} step={2} onChange={setSpeed} /><Slider label="Launch angle (°)" value={angle} min={10} max={80} step={2} onChange={setAngle} /></View><View style={{ flexDirection: "row", gap: 8 }}><View style={{ flex: 1 }}><PrimaryButton label={running ? "Pause" : "Play"} onPress={() => setRunning((value) => !value)} /></View><View style={{ flex: 1 }}><SecondaryButton label="Reset" onPress={() => { setSpeed(18); setAngle(42); setRunning(false); }} /></View></View></Card><Card style={{ marginTop: 14 }}><Text style={{ color: colors.muted, fontSize: 12, fontWeight: "800" }}>DETERMINISTIC ENGINE · VERIFIED</Text><View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: 14 }}><View><Text style={{ color: colors.muted }}>Flight time</Text><Text style={{ color: colors.foreground, fontSize: 20, fontWeight: "800" }}>{result.flightTime.toFixed(2)} s</Text></View><View><Text style={{ color: colors.muted }}>Range</Text><Text style={{ color: colors.foreground, fontSize: 20, fontWeight: "800" }}>{result.range.toFixed(2)} m</Text></View><View><Text style={{ color: colors.muted }}>Peak</Text><Text style={{ color: colors.foreground, fontSize: 20, fontWeight: "800" }}>{result.peakHeight.toFixed(2)} m</Text></View></View><View style={{ marginTop: 16 }}><PrimaryButton label="Mark lab complete" onPress={() => void recordLabCompletion()} /></View></Card></ScrollView></ScreenContainer>;
}
