...
import { deleteDraft, recordAttempt } from "@/lib/progress-store";
...
import { activePracticeIndexForConcept, getActivePracticeQuestions } from "@/lib/mock/adapters/catalog";
...
import { loadPuzzleEvidenceWithStatus, loadResolvedPuzzleIdsWithStatus, missedPuzzleReviewQueue, recordPuzzleEvidence, recordReviewMastery, resolvePuzzleEvidence } from "@/lib/puzzle-evidence";
import { loadPreferences } from "@/lib/preferences";
import { xpFor } from "@/lib/gamification";
import { practiceScreenModel } from "@/lib/mock/ai/ai-screen-adapters";
import { PremiumExamCard } from "@/components/monetization";
import { useEntitlements } from "@/hooks/use-monetization";
import { PracticeFeedbackCard, PracticeHeader, PracticeHintSheet } from "@/components/practice/PracticeChrome";
...
export default function PracticeScreen() {
  ...
  const questions = useMemo(() => {
    ...
    return catalog.length ? catalog : practiceQuestions;
  }, []);
  const requestedIndex = typeof conceptId === "string" ? (activePracticeIndexForConcept(conceptId) ?? practiceQuestionIndexForConcept(conceptId)) : null;
  ...
  const question = questions[index % Math.max(1, questions.length)] ?? practiceQuestions[0];
  ...
  const aiPractice = practiceScreenModel(question.conceptId);
  const practiceModel = buildPracticeViewModel({ index, feedback: feedback ?? "idle", expected });
  ...
  const next = async () => { if (submitting) return; if (isReviewMode && reviewCursor + 1 < reviewItems.length) { const nextCursor = reviewCursor + 1; setReviewCursor(nextCursor); setIndex(practiceQuestionIndexForConcept(reviewItems[nextCursor].topic ?? "") ?? 0); } else if (isReviewMode) { router.replace("/progress" as never); return; } else setIndex((value) => value + 1); setAnswer(""); setFeedback(null); setHint(false); setHintSheet(false); setHintRevealed(1); resetPuzzle(); const deleted = await persistSafely(deleteDraft("practice")); setPersistenceMessage(localizedPersistenceMessage(deleted)); };
  return <ScreenContainer className="p-5"><ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{ paddingBottom: 32 }}><SectionHeader title={isReviewMode ? tr("practice.reviewMode") : tr("practice.title")} subtitle={isReviewMode && reviewItems.length ? tr("practice.reviewProgress", { current: reviewCursor + 1, total: reviewItems.length }) : tr("practice.subtitle")} /><View style={{ marginBottom: 12 }}><PracticeHeader model={practiceModel} /></View><View style={{ marginBottom: 12 }}><PremiumExamCard locked={!has("exam_prep")} onPress={() => router.push({ pathname: "/paywall", params: { from: "/practice", variant: "exam_focused", feature: "exam_prep" } } as never)} /></View>{isReviewMode && reviewDataFallback && <Text accessibilityLiveRegion="polite" style={{ marginBottom: 10, color: colors.warning, lineHeight: 21 }}>{tr("practice.reviewDataFallback")}</Text>}<DraftRecovery id="practice" onResume={(saved) => { if (typeof saved.data.answer === "string") setAnswer(saved.data.answer); }} /><DraftStatus status={draftStatus} /><Card><Pill label={question.concept.toUpperCase()} active /><Text style={{ marginTop: 14, fontSize: 21, lineHeight: 29, fontWeight: "800", color: colors.foreground }}>{question.prompt}</Text><Text style={{ marginTop: 12, color: colors.muted }}>{tr("practice.answerGuidance", { unit: question.unit })}</Text><View style={{ flexDirection: "row", alignItems: "center", gap: 8, marginTop: 18 }}><TextInput accessibilityLabel={`Answer in ${question.unit}`} value={answer} onChangeText={setAnswer} keyboardType="decimal-pad" placeholder={tr("practice.answerPlaceholder")} placeholderTextColor={colors.muted} style={{ flex: 1, borderWidth: 1, borderColor: colors.border, borderRadius: 14, padding: 14, color: colors.foreground, fontSize: 18 }} /><Text style={{ color: colors.muted, fontWeight: "700" }}>{question.unit}</Text></View><View style={{ flexDirection: "row", gap: 10, marginTop: 14 }}><View style={{ flex: 1 }}><PrimaryButton label={submitting ? tr("common.saving") : tr("practice.checkAnswer")} disabled={submitting} onPress={() => void submit()} /></View><View style={{ flex: 1 }}><SecondaryButton label={hint ? tr("practice.hintShown") : tr("practice.getHint")} onPress={() => { setHint(true); setHintSheet(true); }} /></View></View>{hint && <View style={{ marginTop: 14, padding: 12, borderRadius: 12, backgroundColor: colors.primary + "14" }}><Text style={{ color: colors.primary, lineHeight: 20 }}>{tr("practice.hintBody")}</Text></View>}{feedback && <View style={{ marginTop: 16 }}><Text accessibilityLiveRegion={feedbackAnnouncement?.accessibilityLiveRegion} accessibilityRole="text" style={{ color: feedback === "correct" ? colors.success : colors.warning, fontSize: 18, fontWeight: "800" }}>{feedbackAnnouncement?.message}</Text><Text style={{ marginTop: 5, color: colors.muted }}>{tr("practice.expected", { value: expected.toFixed(2), unit: question.unit })}</Text>{feedback && aiPractice && <View style={{ marginTop: 12 }}><Text style={{ color: colors.primary, fontWeight: "800" }}>{aiPractice.demoLabel}</Text><Text style={{ marginTop: 6, color: colors.foreground, lineHeight: 20 }}>{aiPractice.feedback.whatWasRight} {aiPractice.feedback.whatNeedsWork}</Text><Text style={{ marginTop: 4, color: colors.muted }}>{aiPractice.feedback.nextStep}</Text></View>}{feedback === "correct" && <RevealBlock preferences={{ reducedMotion }}><View accessibilityLabel={tr("practice.xpRewardAccessibility", { xp: xpFor("practice") })} style={{ flexDirection: "row", alignItems: "center", gap: 7, alignSelf: "flex-start", marginTop: 12, paddingHorizontal: 10, paddingVertical: 7, borderRadius: 999, backgroundColor: colors.success + "18", borderWidth: 1, borderColor: colors.success + "40" }}><MaterialIcons name="stars" size={16} color={colors.success} /><Text style={{ color: colors.success, fontWeight: "900" }}>{tr("practice.xpReward", { xp: xpFor("practice") })}</Text></View></RevealBlock>}{feedback === "correct" && <View style={{ marginTop: 12 }}><PrimaryButton label={isReviewMode && reviewCursor + 1 >= reviewItems.length ? tr("practice.returnToProgress") : tr("practice.nextQuestion")} onPress={next} /><View style={{ marginTop: 10 }}><SecondaryButton label={tr("practice.saveNotebook")} onPress={() => void saveToNotebook()} /></View>{notebookAnnouncement && <Text accessibilityLiveRegion={notebookAnnouncement.accessibilityLiveRegion} style={{ marginTop: 8, color: colors.success }}>{notebookAnnouncement.message}</Text>}</View>}</View>}{persistenceMessage && <Text accessibilityLiveRegion="assertive" style={{ marginTop: 10, color: colors.warning }}>{persistenceMessage}</Text>}
      ...
      {feedback ? <View style={{ marginTop: 12 }}><PracticeFeedbackCard model={practiceModel} onNext={() => void next()} nextLabel={isReviewMode && reviewCursor + 1 >= reviewItems.length ? tr("practice.returnToProgress") : tr("practice.nextQuestion")} /></View> : null}
      <PracticeHintSheet visible={hintSheet} hints={practiceModel.hints} revealed={hintRevealed} onReveal={() => setHintRevealed((value) => Math.min(practiceModel.hints.length, value + 1))} onClose={() => setHintSheet(false)} reducedMotion={reducedMotion} />
