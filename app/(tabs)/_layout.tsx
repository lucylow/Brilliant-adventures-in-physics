...
export default function TabLayout() {
  ...
  return (
    <Tabs screenOptions={{ headerShown: false, tabBarActiveTintColor: colors.primary, tabBarInactiveTintColor: colors.muted, tabBarButton: HapticTab, tabBarStyle: { paddingTop: 8, paddingBottom: bottomPadding, height: 56 + bottomPadding, backgroundColor: colors.background, borderTopColor: colors.border, borderTopWidth: 0.5 } }}>
      ...
      <Tabs.Screen name="tutor" options={{ title: "Tutor", tabBarIcon: ({ color }) => <IconSymbol size={25} name="message.fill" color={color} /> }} />
      <Tabs.Screen name="lab" options={{ title: "Lab", tabBarIcon: ({ color }) => <IconSymbol size={25} name="atom" color={color} /> }} />
      <Tabs.Screen name="practice" options={{ title: "Practice", tabBarIcon: ({ color }) => <IconSymbol size={25} name="checkmark.circle.fill" color={color} /> }} />
