...
export type LearningState = { attempts: number; correct: number; lastTopic?: string; savedQuestions: string[]; topics: Record<string, TopicMastery>; lastStudyDate?: string; streak: number };
export type SessionDraft<T> = { id: string; data: T; updatedAt: number };
const EMPTY: LearningState = { attempts: 0, correct: 0, savedQuestions: [], topics: {}, streak: 0 };
...
export async function saveQuestion(question: string): Promise<LearningState> { const current = await loadLearningState(); const savedQuestions = current.savedQuestions.includes(question) ? current.savedQuestions : [...current.savedQuestions, question]; const next = { ...current, savedQuestions }; await AsyncStorage.setItem(STORAGE_KEY, JSON.stringify(next)); return next; }
