...
function isValidDraftRecord(value: unknown): value is Record<string, SessionDraft<unknown>> {
  if (!value || typeof value !== "object" || Array.isArray(value)) return false;
  return Object.entries(value).every(([key, draft]) => key.length > 0 && isValidDraft(draft) && draft.id === key);
}
