/**
 * Compatibility layer for the original PhysicaAI / B.A.V. monetization contracts.
 * New code should prefer the typed entitlement and billing APIs re-exported from index.ts.
 * Existing tests and screens keep importing these symbols unchanged.
 */

export type Tier = "free" | "plus" | "family" | "education" | "lifetime";
...
export const VALID_TIERS: readonly Tier[] = ["free", "plus", "family", "education", "lifetime"];
export const VALID_BILLING_STATES: readonly BillingState[] = ["active", "trial", "grace", "expired", "canceled", "pending"];
...
export type StoreProductPeriod = "month" | "year" | "lifetime";
export type StoreProduct = { id: string; title: string; priceLabel: string; period: StoreProductPeriod };
...
export function consume(usage: { used: number; limit: number }, cost = 1) {
  if (!Number.isFinite(usage.used) || !Number.isFinite(usage.limit) || !Number.isFinite(cost) || usage.used < 0 || usage.limit <= 0 || cost <= 0) {
...
export function isUsable(state: BillingState): boolean {
...
export function buildPaywall(context: "scan" | "tutor" | "lab" | "exam") {
  ...
  return {
    title: "Unlock deeper physics learning",
    subtitle: "Core learning stays free. Upgrade when premium tools help.",
    benefits,
    primary: "See plans",
    secondary: "Not now",
  };
...
export const ethicalCopy = {
  noFakeScarcity: true,
  noForcedCountdowns: true,
  clearRenewal: true,
  clearPricing: true,
  easyDismiss: true,
} as const;
...
export const VALID_PERIODS: readonly StoreProductPeriod[] = ["month", "year", "lifetime"];
...
export function isValidProduct(product: Product): boolean {
  ...
  return (
    ...
    VALID_PERIODS.includes(candidate.period as StoreProductPeriod) &&
...
export function purchaseReducer(state: PurchaseSession, action: PurchaseAction): PurchaseSession {
  switch (action.type) {
    case "START":
      return { state: "loading", productId: action.productId };
    case "PENDING":
      return { ...state, state: "pending" };
    case "SUCCESS":
      return { ...state, state: "success", error: undefined };
    case "CANCEL":
      return { ...state, state: "cancelled", error: undefined };
    case "RESTORE":
      return { state: "restoring" };
    case "ERROR":
      return { ...state, state: "failed", error: action.message };
    default:
      return state;
  }
}
    ...
    (
      {
        ...
        timeout: "The store took too long to respond. Try again.",
        not_allowed: "Purchases are unavailable on this device.",
        unknown: "We couldn't complete the purchase.",
      } as Record<string, string>
    )[code] ?? "We couldn't complete the purchase."
  );
}
